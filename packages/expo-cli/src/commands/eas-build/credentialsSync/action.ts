...
export default async function credentialsSyncAction(projectDir: string, options: Options) {
  ...
  Analytics.logEvent(analyticsEvents.CREDENTIALS_SYNC_COMMAND, anaylyticsCtx);
  if (update === 'local') {
    await updateLocalCredentialsAsync(projectDir, platform, anaylyticsCtx);
  } else {
    await updateRemoteCredentialsAsync(projectDir, platform, anaylyticsCtx);
  }
}
...
async function updateRemoteCredentialsAsync(
  ...
  anaylyticsCtx: Record<string, string>
): Promise<void> {
  ...
  if ([BuildCommandPlatform.ALL, BuildCommandPlatform.ANDROID].includes(platform)) {
    try {
      ...
      Analytics.logEvent(analyticsEvents.CREDENTIALS_SYNC_UPDATE_REMOTE_SUCCESS, {
        ...anaylyticsCtx,
        platform: 'android',
      });
    } catch (error) {
      Analytics.logEvent(analyticsEvents.CREDENTIALS_SYNC_UPDATE_REMOTE_FAIL, {
        ...anaylyticsCtx,
        platform: 'android',
...
export async function updateLocalCredentialsAsync(
  ...
  if ([BuildCommandPlatform.ALL, BuildCommandPlatform.ANDROID].includes(platform)) {
    try {
      ...
      Analytics.logEvent(analyticsEvents.CREDENTIALS_SYNC_UPDATE_LOCAL_SUCCESS, {
        ...anaylyticsCtx,
        platform: 'android',
      });
    } catch (error) {
      Analytics.logEvent(analyticsEvents.CREDENTIALS_SYNC_UPDATE_LOCAL_FAIL, {
        ...anaylyticsCtx,
        platform: 'android',
        reason: error.message,
      });
